import re
input = """beacf afbd bcead cgefa ecdbga efb gbfdeac ecgfbd acbdfe fb | bf efb bgecdfa egcfa
...
def is_unique(length):
    return length in uniques

def get_unique_digit_by_length(length):
...
def get_decoded_segments(signals_str):
    signals = signals_str.split(' ')
    # create a dict for encoding the mixed segments
    encoded_segments = {}

    # sort by length desc
    signals.sort(key=len, reverse=True)

    # extract the 1, 7 & 4 signals
    one = signals.pop()  # ab
    seven = signals.pop()  # dab
    four = signals.pop()  # eafb

    # subtract one's segments from seven will reveal the 'a' segment
    reg = "[" + one + ']'
    a = re.sub(reg, "", seven);
    encoded_segments[a] = 'a' # a is now d

    # subtracting one's segments from 4 will give the 'b' & 'd' segments
    bd = re.sub(reg, "", four)  # bd => 'ef'

    # from the 5 lengths signal get the one that is a five - only five is contructed by b & d segments
    # the last 3 items in signals are a 5 length signals
    [five] = [elem for elem in signals[-3:] if elem.find(bd[0]) > -1 and elem.find(bd[1]) > -1]
    signals.remove(five)
    ...
    # compare five's segments to one's segments (cf) will reveal which one is each, if the first char exists it's an f
    if bool(re.search(one[0], five)):
        f = one[0]
        c = one[1]
    else:
        f = one[1]
    ...
    encoded_segments[f] = 'f'
    encoded_segments[c] = 'c'

    # get the g segment
    reg = "[" + a + f + bd[0] + bd[1] + "]"
    g = re.sub(reg, "", five)
    encoded_segments[g] = 'g'

    # get 3 - only 3 has segments c & f
    [three] = [elem for elem in signals[-2:] if
               elem.find(c) > -1 and elem.find(f) > -1]
    signals.remove(three)
    # get the d segment by extracting a,c,f,g
    reg = '[' + a + c + f + g + ']'
    d = re.sub(reg, "", three)
    encoded_segments[d] = 'd'

    # get b segment
    b = re.sub(d, "", bd)
    encoded_segments[b] = 'b'

    # finally, pop out the last 5length item - 2, and extract the e segment
    reg = '[' + a + c + d + g + ']'
    e = re.sub(reg, "", signals.pop())
    encoded_segments[e] = 'e'

    return encoded_segments


def part2():
    # real digits map, only need the un-uniques ones
    digits = {
        'abcefg': 0,
        'acdeg': 2,
        'acdfg': 3,
    ...
    for line in lines:
        ...
        encoded_segments = get_decoded_segments(signals_str)
        outputs = output_str.split(' ')
        output_value = ''

        for output in outputs:
            if is_unique(len(output)):
                output_value += str(get_unique_digit_by_length(len(output)))
            else:
                decoded = []
                for segment in list(output):
                    decoded.append(encoded_segments[segment])
                decoded.sort()
                digit = ''.join(decoded)
                output_value += str(digits[digit])
