const input = `beacf afbd bcead cgefa ecdbga efb gbfdeac ecgfbd acbdfe fb | bf efb bgecdfa egcfa
...
const uniques = { 2: 1, 3: 7, 4: 4, 7: 8 };
function isUnique(length: number) {
  return !!uniques[length];
}

function getUniqueDigit(length: number) {
  return uniques[length];
}
...
function getDecodeSegments(segmetsStr: string) {
  const signals = segmetsStr.split(' ');
  // arrange by length
  const byLength: Record<number, string[]> = {};
  signals.forEach((signal) => {
    if (!byLength[signal.length]) {
      byLength[signal.length] = [signal];
    } else {
  ...
  const one = byLength[2][0]; // ab
  const seven = byLength[3][0]; // dab
  const four = byLength[4][0]; // eafb

  // subtract one's segments from seven will reveal the 'a' segment
  const a = seven.replace(new RegExp(`[${one}]`, 'g'), '');
  encodedSegments.set(a, 'a');
  // subtracting one's segments from 4 will give the 'b' & 'd' segments
  const bd = four.replace(new RegExp(`[${one}]`, 'g'), ''); // ef
  ...
  let c = '',
    ...
    b = '',
    e = '';
  // start with 5 - because 5 has segments b & d
  const fiveIndex = byLength[5].findIndex((signal) => signal.includes(bd[0]) && signal.includes(bd[1]));
  const five = byLength[5][fiveIndex];
  byLength[5].splice(fiveIndex, 1);
  // compare five's segments to 1's segments (cf) will reveal which one is each, if the first char exists it's an f
  if (new RegExp(`${one[0]}`).test(five)) {
    f = one[0];
    c = one[1];
  } else {
    f = one[1];
  ...
  encodedSegments.set(f, 'f');
  encodedSegments.set(c, 'c');

  // get the g segment
  g = five.replace(new RegExp(`[${a}${f}${bd[0]}${bd[1]}]`, 'g'), '');
  encodedSegments.set(g, 'g');

  // get 3 - only 3 has segments c & f
  const threeIndex = byLength[5].findIndex((signal) => signal.includes(c) && signal.includes(f));
  const three = byLength[5][threeIndex];
  byLength[5].splice(threeIndex, 1);
  // get the d segment by extracting a,c,f,g
  d = three.replace(new RegExp(`[${a}${c}${f}${g}]`, 'g'), '');
  encodedSegments.set(d, 'd');
  // get b segment
  b = bd.replace(new RegExp(d), '');
  encodedSegments.set(b, 'b');

  // finally get the 2
  const two = byLength[5][0];
  e = two.replace(new RegExp(`[${a}${c}${d}${g}]`, 'g'), '');
  encodedSegments.set(e, 'e');
...
function part2() {
  ...
  for (const line of lines) {
    const [signalsStr, outputStr] = line.split(' | ');
    const encodedSegments = getDecodeSegments(signalsStr);
    const outputs = outputStr.split(' ');
    let outputValue = '';
    outputs.forEach((output) => {
      if (isUnique(output.length)) {
        outputValue += getUniqueDigit(output.length);
      } else {
        const decodedValue = output
          .split('')
          .map((segment) => encodedSegments.get(segment))
          .sort()
          .join('');
        outputValue += dict[decodedValue];
      }
    });
